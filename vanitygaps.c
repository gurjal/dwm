...
/*
 * Bottomstack layout + gaps + rmaster
...
bstack(Monitor *m)
{
	...
	if (m->nmaster && n > m->nmaster) {
		sh = (mh - ih) * (m->rmaster ? m->mfact : 1 - m->mfact);
		mh = mh - ih - sh;
		sx = mx;
    sy = m->rmaster ? my + mh + ih : m->wy + oh;
		my = m->rmaster ? m->wy + oh : sy + sh + ih;
	}
...
/*
 * Default tile layout + gaps + rmaster
 */
static void
tile(Monitor *m)
{
